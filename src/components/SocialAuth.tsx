...
const SocialAuth = ({ isNewAccount = false }: SocialAuthProps) => {
  ...
  return (
    <SocialBox>
      <IconButton onClick={() => onSocialClick("google")} size={'large'}>
        ...
        <span>Google 계정으로 {isNewAccount ? '가입하기' : '로그인하기'}</span>
      </IconButton>
      <IconButton onClick={() => onSocialClick("github")} size={'large'}>
        <GitHubLogoIcon /> <span>Github로 {isNewAccount ? '가입하기' : '로그인하기'}</span>
      </IconButton>
    </SocialBox>
  )
}
